...
admin_user_bp = Blueprint("admin_user", __name__, template_folder="../../templates/client/profile")
...
@admin_user_bp.route("/alterar-senha", methods=["POST", "GET"])
@login_required
def change_password():
...
@admin_user_bp.route("/editar", methods=["POST", "GET"])
@login_required
def update_profile():
    if request.method == 'POST':
        action = request.form.get('action')
